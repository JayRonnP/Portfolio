...
class x {
  ...
  #f() {
    ...
    this.#a = setTimeout(() => this.resize(), 100);
  }
  resize() {
...
const X = {
  count: 200,
  colors: [0xa855f7, 0x896abd, 0x3b82f6],
...
function createBallpit(e, t = {}) {
  ...
  function initialize(config) {
    ...
    s = new Z(i.renderer, config);
    i.scene.add(s);
  }
  i.onAfterResize = ev => {
    if (s) {
      s.config.maxX = ev.wWidth / 2;
      s.config.maxY = ev.wHeight / 2;
    }
  };
  i.resize();
  const initialMaxX = i.size.wWidth / 2 || 5;
  const initialMaxY = i.size.wHeight / 2 || 5;
  initialize({ ...t, maxX: initialMaxX, maxY: initialMaxY });
  t.onReady?.();
  const h = S({
    domElement: e,
    onMove() {
  ...
  i.onBeforeRender = frameInfo => {
    if (!c && s) s.update(frameInfo);
  };
  return {
    three: i,
    get spheres() {
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
  ...
  return (
    <canvas
      className={`${className} w-full h-full transition-opacity duration-300 ease-out`}
